...
[`apps/desktop`](../../../../apps/desktop/README.md) is a Tauri 2 application with no compile-time window. Its Rust host starts the real built `dsh web --no-open --host 0.0.0.0 --port 0` entry on all interfaces, preventing the backend from opening the system browser and making the GUI reachable from the LAN, consumes the existing `dsh web:` readiness line (loopback URL plus the optional token-bearing LAN URL), and creates one external-URL WebView after the assembled application settles. The host owns the child process through application exit; teardown kills a live child and waits for it to finish.
...
The Rust host stores `dsh-platform=tauri` and the compile-target `dsh-os` in its owned WebView URL fragment. The fragment remains client-only and survives browser authentication's launch-token redirect to the clean `/` URL. The shared layout renders a 48px drag region only for those markers, then moves the existing sidebar-store toggle into its left edge and binds it to `Command+B` on macOS or `Control+B` elsewhere. The titlebar uses the borderless sidebar fill across its full width, and the conversation region exposes that fill through a 15px top-left corner. macOS uses Tauri's overlay titlebar with native decorations, hidden title text, native traffic lights at the iNotes inset, and system-rounded corners; the HTML brand and control follow the traffic lights. macOS and Windows show the DeepSeek Harness wordmark while the sidebar is expanded, then place the whale in the collapsed control and swap it for the expand icon on hover. The titlebar observes native fullscreen changes because macOS removes the traffic lights there: while fullscreen, the control releases the traffic-light inset and uses the expanded sidebar's 12px content inset or the collapsed rail's 10px icon inset. Windows and Linux use a frameless window with HTML minimize, maximize/restore, and close controls on the right. Browser launches without the markers keep the existing sidebar chrome and do not acquire the desktop shortcut.

The native system tray uses the bundled application icon and owns four menu actions: `显示窗口` restores and focuses the main window, `DSH Web（端口：xxx）` opens the running web application in the default browser using the backend's actual port, `复制访问地址` writes the token-bearing LAN URL (falling back to the loopback URL) to the clipboard so another device can open the same GUI, while `退出程序` synchronously terminates and waits for the Node backend before requesting application exit. A main-window close request hides the window instead of destroying it, so the tray can restore the same WebView and backend session.

## Security and lifecycle

The backend binds to `0.0.0.0` and receives an operating-system-assigned port, deliberately exposing the Web app to the network; browser-session authentication and the process token carried by the printed LAN URL are the fences, and the clipboard LAN URL keeps its token. The WebView URL is accepted only from the readiness line produced by the owned child. Standard Web composition continues to own Host-header trust, API routing, persistence, credentials, and sandbox policy. The `main` WebView receives only Tauri's close, minimize, toggle-maximize, and start-dragging window permissions, scoped to the owned `http://127.0.0.1:*` origin; it receives no filesystem, shell, process, or application command permission.
...
The Rust lifecycle tests pin readiness parsing, fragment markers, and the relative production entry used from installation directories containing spaces, while `desktop:prepare` verifies the published CLI entry after materializing the production closure. Keyless assembled Web snapshots enter through the real launch-token redirect and cover the macOS and Windows titlebars, branding states, and shared sidebar toggle. Existing Web and CLI suites remain the behavior authority because the desktop host deliberately does not fork either implementation.
