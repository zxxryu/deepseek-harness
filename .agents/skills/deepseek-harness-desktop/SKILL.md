...
1. Spawns the real built backend as `dsh web --port 0` on an operating-system-assigned loopback port.
...
4. Appends `dsh-platform=tauri` and `dsh-os=<macos|windows|linux>` to the WebView URL. The shared web layout renders the 48px platform titlebar **only** for those markers; plain browser launches keep the ordinary sidebar control and no window chrome.
...
- `scripts/check-workspace-constraints.ts` — add `'@deepseek-ai/dsh-desktop': ['frontend', 'src-tauri']` to `appPackageFiles`.
- `scripts/verify-runtime-closure.ts` — glob `apps/*/package.json` alongside `packages/*/*` and `vendor/*`; the same change dedupes failure messages through a `Map`.
...
- `packages/client/ui-layout/package.json` — add dependency `@tauri-apps/api`; add `@deepseek-ai/dsh-client-ui-primitives` to peerDependencies and devDependencies (it supplies `BrandWordmark`, `FishLogo`, `IconCloseOutline16`, `IconPanelLeftOutline16`, `Tooltip`).
- `packages/client/ui-layout/src/client/AppFrame.tsx` — `desktopPlatform()` reads `dsh-platform` (must equal `tauri`) and `dsh-os` (`linux` | `macos` | `windows`; throw on any other value) from `window.location.search`. When a platform is present: bind `Cmd+B` (macOS, `metaKey`) / `Ctrl+B` (elsewhere, `ctrlKey`) to `actions.toggleSidebar()` (ignore when alt/shift or a non-`b` key), render `<DesktopTitlebar>` above the frame inside a `.desktopShell` wrapper, and set `data-platform={desktopOs}` on the sidebar column (drives sidebar CSS).
- `packages/client/ui-layout/src/client/DesktopTitlebar.tsx` (new) — 48px `<header>` with `role="toolbar"`, `aria-label` bilingual ("Window controls" / "窗口控制"), `data-tauri-drag-region`, and `gridTemplateColumns: sidebarWidth px + minmax(0,1fr)`. Left cell: the sidebar fold button (bilingual aria-label "Collapse/Open sidebar", tooltip shows `⌘B`/`Ctrl+B`); on Windows, `BrandWordmark` before the button when expanded and `FishLogo` (size 24) inside the button when collapsed. Right cell (`platform !== 'macos'`): minimize, maximize/restore, close buttons calling `@tauri-apps/api/window` `getCurrentWindow()`; close uses `IconCloseOutline16`. Double-click on the drag region (not on a button) toggles maximize. macOS: no window buttons (native traffic lights); observe fullscreen through `desktopWindow.isFullscreen()` + `onResized`, guarded by `'__TAURI_INTERNALS__' in window`, and expose it via `data-fullscreen` so the toggle releases the traffic-light inset while fullscreen.
- `packages/client/ui-layout/src/client/AppFrame.module.css` — `.desktopShell` (titlebar above frame), `.titlebar` (48px, borderless sidebar fill, drag region), `.titlebarSidebar`, `.titlebarMain`, `.windowControls` (absolutely positioned right), `.windowControl`, minimize/maximize icon spans, `.titlebarWordmark` (24px, brand ink, part of the drag region), and the Windows collapsed rules: `[data-platform='windows'][data-sidebar-collapsed]` hides `.titlebarPanelIcon` until hover, when it swaps in and `.titlebarFish` hides. The conversation region exposes the sidebar fill through a 15px top-left corner.
- `packages/client/ui-sidebar/src/client/SidebarRoot.module.css` — `[data-desktop-titlebar][data-platform='windows'] .logoRow { display: none; }` (Windows hides the sidebar's banner logo because the titlebar renders wordmark/whale in both states).

### 5. E2E snapshots and documentation

- `apps/web/tests/desktop-titlebar.e2e.ts` + `apps/web/tests/snapshots/desktop-titlebar/macos-titlebar.expected.md` + `windows-titlebar.expected.md` — keyless assembled coverage: navigate with `?dsh-platform=tauri&dsh-os=macos|windows`, assert the toolbar (drag region, data-platform, no minimize button on macOS), the geometry (titlebar top 0 / height 48, frame top 48, toggle 82px from the left on macOS, 15px center radius), fullscreen inset release, and the Windows wordmark/whale states; also verify the browser path (no markers) renders no titlebar. `captureStableAria` output must match the committed `*.expected.md` snapshots.
...
- The host runs the real `dsh web --port 0` backend — never a mock; installers must stay self-contained (bundled Node, no user Node/pnpm).
