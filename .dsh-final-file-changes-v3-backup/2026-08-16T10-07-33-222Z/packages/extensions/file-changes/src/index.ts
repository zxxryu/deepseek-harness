...
export class FileChangesService extends TypertRemoteService {
  ...
  async undo(request: UndoFileChangeRequest): Promise<UndoFileChangeResult> {
    ...
    if (request.after !== undefined && request.created !== undefined) {
      return await this.undoSnapshot(request.path, resolveOptions, target, info, request.created, request.before, request.after)
    }
    ...
    return await this.undoFragments(target, info, request.diffs)
  }
  ...
  private async undoSnapshot(
    ...
    try {
      await this.ctx.fs.writeText(target, before, { kind: 'replaceIfVersion', version: info.version })
    } catch (error) {
  ...
  private async undoFragments(
    ...
    try {
      await this.ctx.fs.writeText(target, reversed, { kind: 'replaceIfVersion', version: info.version })
    } catch (error) {
      const stale = staleFailure(error)
      if (stale !== undefined) return stale
      throw error
